...
class AuditReport:
    ...
    def add_finding(
        self, dim: str, severity: str, desc: str, location: str, remediation: str
    ):
...
def run_audit(document_path_or_content: str) -> AuditReport:
    ...
    if re.search(r"Do X.*Never do X|Always.*Never", content, re.IGNORECASE):
        report.add_finding(
            "CONSIST",
            "Critical",
            "Negative vs Positive Constraint Clash",
            "Lines 1-100",
            "Add explicit precedence clause",
        )
    ...
    if re.search(r"auth\.js|real_file", content) and "mock" not in content.lower():
        report.add_finding(
            "GROUND",
            "High",
            "Real-looking example without safety token",
            "Line 42",
            "Rename to mock_auth.js",
        )
    ...
    for i, line in enumerate(lines, 1):
        if len(line.strip()) > 120 and not line.strip().startswith(
            ...
            report.add_finding(
                "CLARITY",
                "Medium",
                "Ventilated Prose violation (multiple statements on one line)",
                f"Line {i}",
                "Split to one statement per line",
            )
