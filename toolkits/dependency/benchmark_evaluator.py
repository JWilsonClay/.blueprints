...
def calculate_regression_risk(delta_metrics: Dict) -> float:
    ...
    """
    negatives = [
        v for v in delta_metrics.values() if isinstance(v, (int, float)) and v < 0
    ]
    if not negatives:
        return 0.0
...
def run_benchmark_suite(
    before_metrics: Dict, after_metrics: Dict, content: str = ""
) -> Dict:
    """
    ...
    return {
        ...
        "impact_data": {
            "delta_metrics": delta,
            "pareto_summary": "Improvement detected"
            if not regression
