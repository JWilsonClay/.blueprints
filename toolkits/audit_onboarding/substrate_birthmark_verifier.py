...
def build_metadata_map() -> dict[str, dict]:
    ...
    print("Fetching full metadata from Google Drive (one call for the entire vault)...")
...
def verify_and_inject_seniority(
    filepath: str, meta_map: dict, force_correct: bool = False
):
    ...
    if should_update_created or True:  # we always refresh modified
        ...
        if should_update_created:
            print(
                f"Δ {os.path.basename(filepath)}: Updated {CREATED_FIELD} → {true_created}"
            )
        print(
            f"Δ {os.path.basename(filepath)}: Updated {MODIFIED_FIELD} → {true_updated}"
        )

    else:
        print(
            f"✓ {os.path.basename(filepath)} verified (created={current_created}, modified={data.get(MODIFIED_FIELD)})"
        )
...
def run_seniority_audit(force: bool = False):
    ...
    for root, dirs, files in os.walk(SUBSTRATE_ROOT):
        ...
        for file in files:
            if file.endswith(".md"):
                verify_and_inject_seniority(
                    os.path.join(root, file), meta_map, force_correct=force
                )
