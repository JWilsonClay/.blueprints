...
def align_checklist(file_path: Path, tab_width: int = 4) -> bool:
    ...
    try:
        ...
        try:
            with (
                open(file_path, "r", encoding="utf-8", newline="") as src,
            ...
            with (
                open(file_path, "r", encoding="latin-1", newline="") as src,
                open(temp_path, "w", encoding="utf-8", newline="") as dst,
            ):
                changed_file = process_stream(src, dst, tab_width)
    except Exception:
...
def find_checklists(
    root: Path, pattern: str = "*CHECKLIST.md", ignore_patterns: List[str] = None
) -> List[Path]:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Aligns Markdown tables in project files (table_formatter)."
    )
    parser.add_argument(
        "--pattern",
        default="*CHECKLIST.md",
        help="File pattern to match (default: *CHECKLIST.md)",
    )
    ...
    checklists = find_checklists(
        root, pattern=args.pattern, ignore_patterns=ignore_patterns
    )
    ...
    if args.undo:
        print(
            f"Undo mode: Removing all tabs from {len(checklists)} files matching '{args.pattern}'...\n"
        )
    ...
    print(
        f"Found {len(checklists)} files matching '{args.pattern}'. Starting alignment (Spaces)...\n"
    )
    for file in checklists:
