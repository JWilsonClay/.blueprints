...
def update_file(filepath):
    ...
    if (
        "temp_" in os.path.basename(filepath)
        or "substrate_assimilation_protocol" in filepath
    ):
        return
    ...
    if is_role:
        ...
        new_data["protocol_id"] = data.get(
            "protocol_id", os.path.basename(filepath).replace(".md", "").upper()
        )
    ...
    if "purpose" in data:
        ...
        if purpose_match:
            ...
            new_data["purpose"] = (
                "Execute designated protocol or role functions within the substrate."
            )
    ...
    body = re.sub(
        r"^\*\*?(Protocol ID|Assigned Role|Target Audience|Purpose|Role):\*\*?.*$\n?",
        "",
        body,
