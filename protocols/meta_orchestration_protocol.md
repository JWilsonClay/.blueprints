---
protocol_id: OP-ORCHESTRATE-META
version: 1.0.0
status: ACTIVE
last_updated: 2026-03-02
---
# Operational Protocol: Meta Orchestration (OP-ORCHESTRATE-META)

```json
// EXAMPLE_PROVENANCE_HEADER
{
  "example_genesis_agent_version": "1.0.0",
  "example_timestamp": "2026-03-02T17:07:44-06:00",
  "example_protocol_reference": "OP-ORCHESTRATE-META",
  "example_target_audience": "Verification Agent"
}
```

**Protocol ID:** OP-ORCHESTRATE-META
**Assigned Role:** Orchestrator_Agent
**Target Audience:** All Macro-Agents

## 1. Core Function & Rationale

- This protocol acts as the recursive apex engine to coordinate the architecture without systemic fragmentation.
- This protocol strictly defines hierarchical workflow sequencing paths.
- This protocol implements global parallelism rules and rigid state boundaries.
- This protocol computes universal compute-budget allocation frameworks.
- This protocol governs universal operation termination criteria.

## 2. Execution Constraints

- You MUST receive context payloads from every subordinate protocol across the entire operational plane.
- You MUST ingest outputs and possess native capabilities to forcefully re-invoke any role with enriched state payloads.
- You MUST enforce absolute "loop-detection thresholds" universally across interacting agent behaviors.

## 3. Protocol Evolution & Recursion

- **Condition:** When proposing architectural improvements to underlying system protocols natively.
- **Instruction:** You MUST explicitly operate a "Protocol Evolution" cycle.
- You MUST generate the new candidate protocol file independently as isolated scaffolding.
- You MUST rigorously route the candidate explicitly through the full `audit-refine-test` cycle natively before system-wide adoption.

## 4. Key Orchestration Dimensions

- You MUST validate ecosystem structural alignment against the "Pipeline Self-Improvement" axis.
- You MUST validate ecosystem load capacity against the "Resource Governance" axis.
