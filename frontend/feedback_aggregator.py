...
from .session_manager import update_session  # noqa: E402
...
def record_feedback(user_id: str, agent: str, score: float, context: Dict):
    ...
    log_event(
        "FeedbackAggregator", "frontend_layer", f"Updated learning loop. Avg: {avg:.2f}"
    )
    record_event(success=score > 0.7, tokens=0)


# API
def intake_feedback(uid: str, score: float, reqs: int):
    record_feedback(uid, "Intake", score, {"reqs": reqs})


def planning_feedback(uid: str, score: float, steps: int):
    record_feedback(uid, "Planning", score, {"steps": steps})


def interactive_feedback(uid: str, score: float, fixes: int):
    record_feedback(uid, "Interactive", score, {"fixes": fixes})
